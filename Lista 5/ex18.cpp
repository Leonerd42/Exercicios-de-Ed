...
int main(){
	...
	printf("\n\tEscolha o tempo total do expediente (hr, min, segundos) : ");
	for(int i=0; i<3; i++){
		do{
			scanf("%d",&limite[i]);
		}while(limite[i] < 0);
	...
	printf("\n\tDigite de quanto em quanto tempo os clientes vão entrar (em segundos):");
	do{
		scanf("%d",&entrada);
	}while(entrada <= 0 || entrada > 60);
	
	printf("\n\tOs clientes entrarão na fila a cada %d segundos!",entrada);
	_sleep(1500); 
	...
	while(!flag){
		...
		if(segundos % entrada == 0){
			cl++;
			enfileira(&banco,cl,hora,minutos);			
		}	
		
		if(caixa1){			
			if(desenfileira(&banco,&cx1)){
				//cx1 = aux; 
